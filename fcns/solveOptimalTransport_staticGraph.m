function [X_K, X_buffer, mass_buffer, K, tau, eig2] = ...
    solveOptimalTransport_staticGraph(Qx,Qy,Qxy,X0,NN,mu_star,K,tau,eps,lF,tol)
...
% static neighbors and connectivity
neighbors = knnsearch(X0,X0,'K',NN);
A = zeros(N,N);
for i = 1:N
    Ni = neighbors(i,2:NN);
    for j = 1:length(Ni)
